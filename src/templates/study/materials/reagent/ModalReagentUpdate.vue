<template>
  <modal
    ...
    <div class="header_wrap">
      <h3 class="header">시약 수정</h3>
      <div class="closeButton" @click="$modal.hide('ModalReagentUpdate')"></div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getContent () {
      const data = {}
      const apiURL = `${this.ENV_CUOME}/reagent/${this.id}`
    ...
    async doDelete () {
      if (confirm('삭제하시겠습니까?')) {
        ...
        this.$axios({
          ...
          this.$modal.hide('ModalReagentUpdate')
        }).catch(error => {
    ...
    async doUpdate ($event) {
      ...
      if (await this.$validator.validate()) {
        ...
        const url = `${this.ENV_CUOME}/reagent/${this.id}`

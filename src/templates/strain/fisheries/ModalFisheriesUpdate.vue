<template>
  <modal
    ...
    <div class="header_wrap">
      <h3 class="header">수산업균주 수정</h3>
      <div class="closeButton" @click="$modal.hide('ModalFisheriesUpdate')"></div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getContent () {
      const content = await getDoc(doc(firestore, '수산업균주', this.id))
      this.modalForm = content.data()
    },
    async doDelete () {
      if (confirm('삭제하시겠습니까?')) {
        await deleteDoc(doc(firestore, '수산업균주', this.id))
        this.$toast.success(
          '삭제되었습니다.',
          this.ToastSettings
        )
        this.$modal.hide('ModalFisheriesUpdate')
      }
    },
    async doUpdate () {
      await setDoc(doc(firestore, '수산업균주', this.id), this.modalForm)
      this.initData()
      this.$toast.success(
        '수정되었습니다.',
        this.ToastSettings

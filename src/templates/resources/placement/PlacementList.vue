<template>
  <main>
    ...
    <div class="action_wrap">
      <button type="button" class="btn primary" @click="showModalPlacementCreate">등록</button>
    ...
    <div class="table_wrap table-hover table_wrap-scoll-y">
      ...
      <div class="table_scroll">
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in contents" :key="index" @click="showModalPlacementUpdate(item.id)">
              <Td>{{ item.id }}</Td>
              <Td>{{ item.name }}</Td>
              <Td>{{ item.memo }}</Td>
            </tr>
    ...
    <ModalPlacementCreate @callback="getContents" />
    <ModalPlacementUpdate :id="selectedId" @callback="getContents" />
  </main>
</template>

<script>

import ModalPlacementCreate from './ModalPlacementCreate'
import ModalPlacementUpdate from './ModalPlacementUpdate'
...
export default {
  ...
  components: {
    ModalPlacementCreate,
    ModalPlacementUpdate,
  },
  data () {
    return {
      selectedId: '',
  ...
  methods: {
    ...
    showModalPlacementCreate () {
      this.$modal.show('ModalPlacementCreate')
    },
    showModalPlacementUpdate (id) {
      this.selectedId = id
      this.$modal.show('ModalPlacementUpdate')
    },
    getContents () {
      this.$Progress.start()
      this.$axios

<template>
  <modal
    ...
    <div class="header_wrap">
      ...
      <div class="closeButton" @click="$modal.hide('ModalPlacementUpdate')"></div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getContent () {
      const data = {}
      const apiURL = `${this.ENV_CUOME}/placement/${this.id}`
    ...
    async doDelete () {
      if (confirm('삭제하시겠습니까?')) {
        ...
        this.$axios({
          ...
          this.$modal.hide('ModalPlacementUpdate')
        }).catch(error => {
    ...
    async doUpdate ($event) {
      ...
      if (await this.$validator.validate()) {
        ...
        const url = `${this.ENV_CUOME}/placement/${this.id}`
